...
function register(){
	...
	$sql = "INSERT 
		...
		if(mysqli_query($conn,$sql)) 
		{
			...
			echo "<script>alert(\"Resgistration Error\")/script>";
		}
	/*if (mysqli_query($conn,$sql)) {
...
if(mysqli_num_rows($result)>=1){
   ...
   if ($result) {
	...
	if (mysqli_query($conn,$sql)) {
	  ...
	  $sql2="INSERT INTO partnerprefs VALUES($id, $id,0,0,'','',0,'','','','','','','','','')";
	  mysqli_query($conn,$sql2);
	  echo "<script> window.location=\"userhome.php?id=$id\"</script>";
	  $sql="UPDATE TABLE users SET profilestat=1 WHERE id=$id";
	} else {
	  echo "Error: " . $sql . "<br>" . $conn->error;
	}
}
...
if(mysqli_num_rows($result) == 0) {
     // no photo for curret user, do stuff...
		$sql="INSERT INTO photos (id, cust_id, pic1, pic2, pic3, pic4) VALUES ($id, $id, 'images/$pic1' ,'images/$pic2', 'images/$pic3','images/$pic4')";
...
if(move_uploaded_file($_FILES['pic1']['tmp_name'], $target1)&&move_uploaded_file($_FILES['pic2']['tmp_name'], $target2)&&move_uploaded_file($_FILES['pic3']['tmp_name'], $target3)&&move_uploaded_file($_FILES['pic4']['tmp_name'], $target4))
{
	...
	echo "<script>alert(\"Successfully uploaded pictures\")</script>";
	echo "<script> window.location=\"userhome.php?id=$id\"</script>";
...
echo "Sorry there was problem uploading your photo";
}
